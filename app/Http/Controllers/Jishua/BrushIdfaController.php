<?php
namespace App\Http\Controllers\Jishua;

use App\Http\Controllers\Controller;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class BrushIdfaController extends Controller
{
    public function get(
        Request $request
    ) {
        $idfa      = $request->input('idfa');
        $device_id = $request->input('device_id');
        if (!$idfa || !$device_id) {
            return $this->fail_response(['message' => '缺少参数cb_params']);
        }

        // // 查询手机的手机组id
        $device_unique = $this->get_device_unique();
        $data          = ['device_unique' => $device_unique];
        $brush_mobiles = DB::table('brush_mobiles')->where($data)->select('mobile_group_id')->first();
        if (!$brush_mobiles) {

            // 添加手机
            DB::table('brush_mobiles')->insert($data);
            return $this->fail_response(['message' => 'new mobile']);

        } else {
            $mobile_group_id = $brush_mobiles->mobile_group_id;
            if ($mobile_group_id == 0) {
                return $this->fail_response(['message' => 'this mobile no task']);
            }
        }

        // 根据一些条件查询任务
        $now_date = date('Y-m-d H:i:s');
        $response = DB::table('brush_idfas')
            ->where([
                ['is_brushing', '=', 1],
                ['start_time', '<=', $now_date],
                ['end_time', '>=', $now_date],
                ['mobile_group_id', '=', $mobile_group_id],
            ])
            ->whereColumn([
                ['success_idfa_num', '<', 'order_num'],
            ])
            ->first();
        if (!$response) {
            return $this->fail_response(['message' => 'no brush idfa task']);
        }
        $response->ret = 0;

        // 创建任务 brush_idfa_tasks
        $brush_idfa_id = $response->id;
        $appid         = $response->appid;
        $tid           = DB::table('brush_idfa_tasks')->insertGetId([
            'idfa'          => $idfa,
            'appid'         => $appid,
            'device_id'     => $device_id,
            'brush_idfa_id' => $brush_idfa_id,
        ]);

        $response->tid = $tid;
        $response->bid = $brush_idfa_id;

        // 统计获取成功的次数
        DB::table('brush_idfas')->where('id', $brush_idfa_id)->increment('returned');

        $cb_params = json_encode(compact('idfa', 'device_id'));

        // 回调任务，拼接回调地址
        if ($response->apiType == 1) {
            $callback_url       = urlencode(url('backend/notify_success?tid=' . $tid . '&appid=' . $appid . '&bid=' . $brush_idfa_id . '&check_token=' . base64_encode($cb_params)));
            $response->callback = str_replace('{callback}', $callback_url, $response->callback);
        }

        return response()->json($response);
    }

    public function notifySuccess(
        Request $request
    ) {
        $check_token   = $request->input('check_token', '');
        $device_id     = $request->input('device_id', '');
        $brush_idfa_id = $request->input('bid', '');
        $task_id       = $request->input('tid', '');
        $appid         = $request->input('appid', '');
        $idfa          = $request->input('idfa', '');
        if ($check_token) {
            $check_token = json_decode(base64_decode($check_token));
            if ($check_token) {
                $device_id = $check_token->device_id;
                $idfa      = $check_token->idfa;
            }
        }
        if (!$task_id || !$brush_idfa_id) {
            return $this->fail_response(['message' => 'no task_id and brush_idfa_id']);
        }

        DB::table('brush_idfa_tasks')->where('id', $task_id)->update(['task_status' => 1]);

        // 统计成功激活次数
        DB::table('brush_idfas')->where('id', $brush_idfa_id)->increment('success_idfa_num');

        return $this->success_response(['success' => 1]);
    }

    public function ciliuGet(Request $request)
    {
        $device_unique = $this->get_device_unique();
        $data          = ['device_unique' => $device_unique];
        $brush_mobiles = DB::table('brush_mobiles')->where($data)->select('mobile_group_id')->first();
        if (!$brush_mobiles) {

            // 添加手机
            DB::table('brush_mobiles')->insert($data);
            return $this->fail_response(['message' => 'new mobile']);

        } else {
            $mobile_group_id = $brush_mobiles->mobile_group_id;
            if ($mobile_group_id == 0) {
                return $this->fail_response(['message' => 'this mobile no task']);
            }
        }

        // 获取有次留量的任务
        $brush_idfa_id = DB::table('brush_idfas')->select('id')
            ->where([
                ['is_brushing', '=', '1'],
                ['is_ciliu', '=', '1'],
                ['mobile_group_id', '=', $mobile_group_id],
            ])
            ->whereColumn([
                ['ciliu_returned_success', '<', 'ciliu_return_num'],
            ])->value('brush_idfa_id');
        if (!$brush_idfa_id) {
            return $this->fail_response(['message' => 'ciliu task finished']);
        }

        // todo 混淆获取id
        $brush_idfa_task = DB::table('brush_idfa_tasks')
            ->where('brush_idfa_id', $brush_idfa_id)
            ->where('task_status', 1)
        //    ->where('created_at', '>', date('Y-m-d', strtotime('-1 days')))
            ->limit(1)
            ->first();
        if (!$brush_idfa_task) {
            return $this->fail_response(['message' => 'no more brush_idfa_task']);
        }

        DB::table('brush_idfa_tasks')->where('id', $brush_idfa_task->id)->increment('task_status');

        // 统计获取次留次数
        DB::table('brush_idfas')->where('id', $brush_idfa_task->brush_idfa_id)->increment('ciliu_returned');

        $brush_idfa = DB::table('brush_idfas')->select('bundleId', 'process')->where('id', $brush_idfa_task->brush_idfa_id)->first();

        $brush_idfa_task->bundleId = $brush_idfa->bundleId;
        $brush_idfa_task->process  = $brush_idfa->process;

        return $this->success_response((array) $brush_idfa_task);
    }

    public function ciliuReport(Request $request)
    {
        $id     = $request->input('id', '');
        $status = $request->input('status', '');

        if ($status === '' || !$id) {
            return $this->fail_response(['message' => 'que shao id canshu']);
        }

        $brush_idfa_task = DB::table('brush_idfa_tasks')->select('brush_idfa_id')->where('id', $id)->first();

        if ($status == 1) {
            // 统计上报次数
            DB::table('brush_idfas')->where('id', $brush_idfa_task->brush_idfa_id)->increment('ciliu_returned_success');
        }

        // 变更任务状态
        $res = DB::table('brush_idfa_tasks')->where('id', $id)->increment('task_status', 1, ['status' => $status]);
        if ($res) {
            return $this->success_response((array) $brush_idfa_task);
        } else {
            return $this->fail_response();
        }
    }

    public function get2(
        Request $request
    ) {
        $idfa      = $request->input('idfa');
        $device_id = $request->input('device_id');
        if (!$idfa || !$device_id) {
            return $this->fail_response(['message' => '缺少参数cb_params']);
        }

        // 查询手机的当前时间的任务
        $now_date  = date('Y-m-d H:i:s');
        $mobile_id = $this->get_mobile_id('active');
        if (!$mobile_id) {
            return $this->fail_response(['message' => 'new mobile']);
        }
        $mobile_assign_active = DB::table('mobile_assign_active')->where([
            ['mobile_id', '=', $mobile_id],
            ['start_time', '<=', $now_date],
            ['end_time', '>', $now_date],
        ])
            ->select('id', 'active_task_id')
            ->first();
        if (!$mobile_assign_active) {
            // 没有任务
            return $this->fail_response(['message' => 'this mobile no task']);
        }
        $task_id = $mobile_assign_active->active_task_id;

        // 根据一些条件查询任务
        $response = DB::table('brush_idfas_active')
            ->whereColumn([
                ['success_idfa_num', '<', 'order_num'],
            ])
            ->find($task_id);
        if (!$response) {
            // 标志已完成
            DB::table('brush_idfas_active')->where('id', $task_id)->update([
                'is_finish' => 1,
            ]);
            DB::table('mobile_assign_active')->where('id', $mobile_assign_active->id)->delete();

            return $this->fail_response(['message' => 'no brush idfa task']);
        }
        $response->ret = 0;

        // 创建任务 brush_idfa_tasks
        $brush_idfa_id = $response->id;
        $appid         = $response->appid;
        $tid           = DB::table('brush_idfa_tasks2')->insertGetId([
            'idfa'          => $idfa,
            'appid'         => $appid,
            'device_id'     => $device_id,
            'brush_idfa_id' => $brush_idfa_id,
        ]);

        $response->tid = $tid;
        $response->bid = $brush_idfa_id;

        // 统计获取成功的次数
        DB::table('brush_idfas_active')->where('id', $brush_idfa_id)->increment('returned');

        $cb_params = json_encode(compact('idfa', 'device_id'));

        // 回调任务，拼接回调地址
        if ($response->apiType == 1) {
            $callback_url       = urlencode(url('backend/notify_success2?tid=' . $tid . '&appid=' . $appid . '&bid=' . $brush_idfa_id . '&check_token=' . base64_encode($cb_params)));
            $response->callback = str_replace('{callback}', $callback_url, $response->callback);
        }

        return response()->json($response);
    }

    public function notifySuccess2(
        Request $request
    ) {
        $check_token   = $request->input('check_token', '');
        $device_id     = $request->input('device_id', '');
        $brush_idfa_id = $request->input('bid', '');
        $task_id       = $request->input('tid', '');
        $appid         = $request->input('appid', '');
        $idfa          = $request->input('idfa', '');
        if ($check_token) {
            $check_token = json_decode(base64_decode($check_token));
            if ($check_token) {
                $device_id = $check_token->device_id;
                $idfa      = $check_token->idfa;
            }
        }
        if (!$task_id || !$brush_idfa_id) {
            return $this->fail_response(['message' => 'no task_id and brush_idfa_id']);
        }

        DB::table('brush_idfa_tasks2')->where('id', $task_id)->update(['task_status' => 1]);

        // 统计成功激活次数
        DB::table('brush_idfas_active')->where('id', $brush_idfa_id)->increment('success_idfa_num');

        return $this->success_response(['success' => 1]);
    }

    public function ciliuGet2(Request $request)
    {
        // 查询手机的当前时间的任务
        $now_date  = date('Y-m-d H:i:s');
        $mobile_id = $this->get_mobile_id('ciliu');
        if (!$mobile_id) {
            return $this->fail_response(['message' => 'new mobile']);
        }
        $mobile_assign_ciliu = DB::table('mobile_assign_ciliu')->where([
            ['mobile_id', '=', $mobile_id],
            ['start_time', '<=', $now_date],
            ['end_time', '>', $now_date],
        ])->select('id', 'ciliu_task_id', 'active_task_id')->first();
        if (!$mobile_assign_ciliu) {
            // 没有任务
            return $this->fail_response(['message' => 'this mobile no task']);
        }
        $ciliu_task_id = $mobile_assign_ciliu->ciliu_task_id;
        // 获取有次留量的任务
        $brush_idfa_id = DB::table('brush_idfas_ciliu')->select('id')
            ->where([
                'id' => $ciliu_task_id,
            ])
            ->select('active_task_id')
            ->whereColumn([
                ['ciliu_returned_success', '<', 'ciliu_return_num'],
            ])->value('active_task_id');
        if (!$brush_idfa_id) {

            // 标志已完成
            DB::table('brush_idfas_ciliu')->where('id', $ciliu_task_id)->update([
                'is_finish' => 1,
            ]);
            DB::table('mobile_assign_ciliu')->where('id', $mobile_assign_ciliu->id)->delete();

            return $this->fail_response(['message' => 'ciliu task finished']);
        }

        // todo 混淆获取id
        $brush_idfa_task = DB::table('brush_idfa_tasks2')
            ->where('brush_idfa_id', $brush_idfa_id)
            ->where('task_status', 1)
            ->where('created_at', '>', date('Y-m-d', strtotime('-1 days')))
            ->limit(1)
            ->first();
        if (!$brush_idfa_task) {
            return $this->fail_response(['message' => 'no more brush_idfa_task']);
        }

        DB::table('brush_idfa_tasks2')->where('id', $brush_idfa_task->id)->increment('task_status');

        // 统计获取次留次数
        DB::table('brush_idfas_ciliu')->where('id', $brush_idfa_task->brush_idfa_id)->increment('ciliu_returned');

        $brush_idfa = DB::table('brush_idfas_active')->select('bundleId', 'process')->where('id', $brush_idfa_task->brush_idfa_id)->first();

        $brush_idfa_task->bundleId = $brush_idfa->bundleId;
        $brush_idfa_task->process  = $brush_idfa->process;

        return $this->success_response((array) $brush_idfa_task);
    }

    public function ciliuReport2(Request $request)
    {
        $id     = $request->input('id', '');
        $status = $request->input('status', '');

        if ($status === '' || !$id) {
            return $this->fail_response(['message' => 'que shao id canshu']);
        }

        $brush_idfa_task = DB::table('brush_idfa_tasks2')->select('brush_idfa_id')->where('id', $id)->first();

        if ($status == 1) {
            // 统计上报次数
            DB::table('brush_idfas_ciliu')->where('active_task_id', $brush_idfa_task->brush_idfa_id)->increment('ciliu_returned_success');
        }

        // 变更任务状态
        $res = DB::table('brush_idfa_tasks2')->where('id', $id)->increment('task_status', 1, ['status' => $status]);
        if ($res) {
            return $this->success_response((array) $brush_idfa_task);
        } else {
            return $this->fail_response();
        }
    }

    public function get_mobile_id($type)
    {
        $mobile_table  = 'brush_' . $type . '_mobiles';
        $device_unique = $this->get_device_unique();
        $data          = ['device_unique' => $device_unique];
        $mobile_id     = DB::table($mobile_table)->where($data)->select('id')->value('id');
        if (!$mobile_id) {
            // 添加手机
            DB::table($mobile_table)->insert($data);
            return false;
        }
        return $mobile_id;
    }
}
